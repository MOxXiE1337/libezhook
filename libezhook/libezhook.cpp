...
unsigned char g_trampoline[0x74] = 
{
...
void __stdcall tl_set_owner(void* trampoline, ezhook * hook)
{
	if (trampoline == NULL || hook == NULL)
		return;

	unsigned char* utrampoline = (unsigned char*)trampoline;
	// set ebx
	*(unsigned int*)(utrampoline + 0x2) = (unsigned int)&hook->ebx;
	// set esi
	*(unsigned int*)(utrampoline + 0x8) = (unsigned int)&hook->esi;
	// set edi
	*(unsigned int*)(utrampoline + 0xE) = (unsigned int)&hook->edi;
	// set original bytes
	*(unsigned int*)(utrampoline + 0x13) = (unsigned int)&hook->original_bytes;
	// set new bytes
	*(unsigned int*)(utrampoline + 0x18) = (unsigned int)&hook->new_bytes;
	// set target function
	*(unsigned int*)(utrampoline + 0x1E) = (unsigned int)&hook->target;
	// set return address
	*(unsigned int*)(utrampoline + 0x3D) = (unsigned int)&hook->ret_addr;
	// set edi
	*(unsigned int*)(utrampoline + 0x5C) = (unsigned int)&hook->edi;
	// set esi
	*(unsigned int*)(utrampoline + 0x62) = (unsigned int)&hook->esi;
	// set ebx
	*(unsigned int*)(utrampoline + 0x68) = (unsigned int)&hook->ebx;
	// set return address
	*(unsigned int*)(utrampoline + 0x6E) = (unsigned int)&hook->ret_addr;
}
...
void* __stdcall hook(void* target, void* detour)
{
	...
	*(unsigned int*)&hook->new_bytes[1] = offset;
	...
	// create a trampoline 
	void* trampoline = VirtualAlloc(NULL, 0x74, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
	...
	// copy shell code
	memcpy(trampoline, g_trampoline, 0x74);

	// set trampoline's owner
	tl_set_owner(trampoline, hook);

	// set hook's trampoline
...
void __stdcall unhook(void* target, void* detour)
{
	...
	// free trampoline
	VirtualFree(hook->trampoline, 0x74, MEM_RELEASE);
